...
| VERDE | sin commit — artefacto git | Creó el tag anotado local `v1.0.0` («Entrega final») apuntando a `1f0560e`, exactamente según las notas técnicas de la historia. Filtro 2/2 correctas, suite completa 314 verdes y formato sin diferencias. El push del tag (`git push --tags`) corresponde a la pareja; si llegan commits posteriores antes del merge final, mover el tag mientras no esté publicado: `git tag -fa v1.0.0 -m "Entrega final" <sha>`. |
